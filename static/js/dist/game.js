...
class Player extends AcGameObject {
    ...
    is_attacked(angle, damage) {
        for (let i = 0; i < 20 + Math.random() * 10; i ++ ) {
            ...
            let vx = Math.cos(angle), vy = Math.sin(angle); // 粒子随机往四周飞溅
        ...
        this.damage_x = Math.cos(angle);  // 玩家被击退的方向
    ...
    render_skill_coldtime() {
        ...
        if (this.fireball_coldtime > 0) {
            ...
            this.ctx.arc(x * scale, y * scale, radius * scale, 0 - Math.PI / 2, Math.PI * 2 * (1 - this.fireball_coldtime / 3) - Math.PI / 2, true);
            this.ctx.lineTo(x * scale, y * scale);
            this.ctx.fillStyle = "rgba(0, 0, 255, 0.6)";
            this.ctx.fill();
        }

        x = 1.62, y = 0.9, radius = 0.04;
        ...
        this.ctx.stroke();
        this.ctx.clip();
        this.ctx.drawImage(this.blink_img, (x - radius) * scale, (y - radius) * scale, radius * 2 * scale, radius * 2 * scale);
        this.ctx.restore();

        if (this.blink_coldtime > 0) {
            this.ctx.beginPath();
            this.ctx.moveTo(x * scale, y * scale);
            this.ctx.arc(x * scale, y * scale, radius * scale, 0 - Math.PI / 2, Math.PI * 2 * (1 - this.blink_coldtime / 5) - Math.PI / 2, true);
            this.ctx.lineTo(x * scale, y * scale);
            this.ctx.fillStyle = "rgba(0, 0, 255, 0.6)";
            this.ctx.fill();
        }
...
class MultiPlayerSocket {
    constructor(playground) {
        this.playground = playground;
        this.ws = new WebSocket("wss://app4231.acapp.acwing.com.cn/wss/multiplayer/");
        this.start();
    }
    ...
    receive() {
        ...
        this.ws.onmessage = function(e) {
            ...
            if (uuid === outer.uuid) return false;  //自己不再接收指令
...
class AcGamePlayground {
    ...
    get_random_color() {
        let colors = ["cyan", "scarlet", "lime", "yellow", "aqua", "marron", "navy", "olive", "fuchsia"];
